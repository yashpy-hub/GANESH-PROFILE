...
export class SubAgentScope {
    ...
    static async create(name, runtimeContext, promptConfig, modelConfig, runConfig, toolConfig, outputConfig) {
        if (toolConfig) {
            const toolRegistry = await runtimeContext.getToolRegistry();
            const toolsToLoad = [];
        ...
        try {
            while (true) {
                ...
                if (functionCalls.length > 0) {
                    currentMessages = await this.processFunctionCalls(functionCalls, toolRegistry, abortController, promptId);
                }
                else {
     ...
     */
    async processFunctionCalls(functionCalls, toolRegistry, abortController, promptId) {
        ...
        for (const functionCall of functionCalls) {
            ...
            if (functionCall.name === 'self.emitvalue') {
                ...
                toolResponse = await executeToolCall(this.runtimeContext, requestInfo, toolRegistry, abortController.signal);
            }
